...
{
    public partial class MqttCollectPropertyRazor : IPropertyUIBase
    {
        ...
        {
            var mqttCollectProperty = (MqttCollectProperty)Model.Value;

            if (mqttCollectProperty.TLS == true)
            {
                {
                    var filePath = Path.Combine("PluginFile", Id, nameof(mqttCollectProperty.CAFile));
                    ...
                    mqttCollectProperty.CAFile = fileName;
                }
            }
        }
        private async Task OnClientCertificateFileChange(UploadFile file)
        {
            ...
            {
                {
                    var filePath = Path.Combine("PluginFile", Id, nameof(mqttCollectProperty.ClientCertificateFile));
                    if (!Directory.Exists(filePath))//如果不存在就创建文件夹
                        Directory.CreateDirectory(filePath);
                    //var fileSuffix = Path.GetExtension(file.Name).ToLower();// 文件后缀
                    var fileObjectName = $"{file.File.Name}";//存储后的文件名
                    ...
                    mqttCollectProperty.ClientCertificateFile = fileName;
                }
        ...
        private async Task OnClientKeyFileChange(UploadFile file)
        {
            ...
            if (mqttCollectProperty.TLS == true)
            {
                ...
                {
                    var filePath = Path.Combine("PluginFile", Id, nameof(mqttCollectProperty.ClientKeyFile));
                    if (!Directory.Exists(filePath))//如果不存在就创建文件夹
                        Directory.CreateDirectory(filePath);
                    //var fileSuffix = Path.GetExtension(file.Name).ToLower();// 文件后缀
                    var fileObjectName = $"{file.File.Name}";//存储后的文件名
                    ...
                    mqttCollectProperty.ClientKeyFile = fileName;
                }
            }
        }
        [Inject]
