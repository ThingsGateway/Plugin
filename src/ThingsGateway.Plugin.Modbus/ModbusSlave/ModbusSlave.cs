...
public class ModbusSlave : BusinessReceivedFoundationBase
{
    ...
    public override Type DriverUIType
    {
        get
        {
            if (_plc.Channel?.ChannelType == ChannelTypeEnum.TcpService || _plc?.Channel == null)
    ...
    protected override Task ProtectedExecuteAsync(object? state, CancellationToken cancellationToken)
    {
        ...
        static async PooledTask ProtectedExecuteAsync(ModbusSlave @this, CancellationToken cancellationToken)
        {
            ...
            foreach (var item in list)
            {
                ...
                {
                    await @this._plc.WriteJsonNodeAsync(item.Key, JsonUtil.GetJsonNodeFromObj(variableRuntime.Value), result, variableRuntime.BitConverter, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    await @this._plc.WriteJsonNodeAsync(item.Key, JsonUtil.GetJsonNodeFromObj(variableRuntime.Value), variableRuntime.DataType, variableRuntime.BitConverter, cancellationToken).ConfigureAwait(false);
                }
            }
