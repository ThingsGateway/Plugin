...
public class ThingsGatewayNodeManager : CustomNodeManager2
{
    ...
    private void ChangeNodeData(OpcUaTag tag, object value, DateTime dateTime)
    {
        ...
        try
        {
            ...
            var jsonNode = JsonUtil.GetJsonNodeFromObj(value);
            var dataValue = SystemTextJsonUtil.DecoderObject(
               Server.MessageContext,
           tag.DataType,
                TypeInfo.GetBuiltInType(tag.DataType, SystemContext.TypeTable),
                jsonNode.CalculateActualValueRank(),
                jsonNode
                );
            if (dataValue == null)
            {
                _businessBase.LogMessage?.LogWarning($"{tag.NodeId} value is null , jsonNode: {jsonNode}");
            }
            else
            {
                _businessBase.LogMessage?.LogTrace($"{tag.NodeId} value {dataValue} , jsonNode: {jsonNode}");
            }
            newValue = dataValue;
            success = true;
        }
    ...
    public override void Write(OperationContext context, IList<WriteValue> nodesToWrite, IList<ServiceResult> errors)
    {
        ...
        lock (lockObject)
        {
            ...
            foreach (var item in tags)
            {
                ...
                {
                    writeInfos.Add((variableRuntime, hashSetNodeId[item.Key].Value.Value?.ToSystemTextJsonString()));
                }
            }
                ...
                if (StatusCode.IsGood(opcuaTag.StatusCode))
                {
                    ...
                    if (opcuaTag.Value != null)
                    {
                        var result = GlobalData.RpcService.InvokeDeviceMethodAsync("OpcUaServer - " + context1?.OperationContext?.Session?.Identity?.DisplayName,
                            ...
                            {
                                {
                                    variableRuntime.DeviceName,   new Dictionary<string, JsonNode>() { {opcuaTag.SymbolicName,JsonUtil.GetJsonNodeFromObj( value) } }
                                }
                            }
                            ).GetAwaiter().GetResult();
