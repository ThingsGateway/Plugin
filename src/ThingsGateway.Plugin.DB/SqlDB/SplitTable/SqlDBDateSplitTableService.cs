...
public class SqlDBDateSplitTableService : DateSplitTableService
{
    ...
    public override List<SplitTableInfo> GetAllTables(ISqlClient db, EntityInfo EntityInfo, List<DbTableInfo> tableInfos)
    {
    ...
    public override string GetTableName(ISqlClient db, EntityInfo EntityInfo)
    {
        var type = (SplitType)_sqlDBProducerProperty.SqlDBSplitType;
        return GetTableName(db, EntityInfo, type);
    }

    public override string GetTableName(ISqlClient db, EntityInfo EntityInfo, SplitType splitType)
    {
        var date = db.GetDate();
        return GetTableNameByDate(EntityInfo, splitType, date);
    }

    public override string GetTableName(ISqlClient db, EntityInfo entityInfo, SplitType splitType, object fieldValue)
    {
    ...
    {
        if (!dbTableName.Contains("{year}")) throw new SqlOrmException("table name need {{year}}", "分表表名需要占位符 {{year}}");
        if (!dbTableName.Contains("{month}")) throw new SqlOrmException("table name need {{month}}", "分表表名需要占位符 {{month}} ");
        if (!dbTableName.Contains("{day}")) throw new SqlOrmException("table name need {{day}}", "分表表名需要占位符{{day}}");
        if (_yearRegex.Count(dbTableName) > 1) throw new SqlOrmException(" There can only be one {{year}}", " 只能有一个 {{year}}");
        if (_monthRegex.Count(dbTableName) > 1) throw new SqlOrmException("There can only be one {{month}}", "只能有一个 {{month}} ");
        if (_dayRegex.Count(dbTableName) > 1) throw new SqlOrmException("There can only be one {{day}}", "只能有一个{{day}}");
        if (_regex.IsMatch(dbTableName)) throw new SqlOrmException(" '{{' or  '}}'  can't be numbers nearby", "占位符相令一位不能是数字,比如 : 1{{day}}2 错误 , 正确: 1_{{day}}_2");
    }
