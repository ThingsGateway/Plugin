...
#endif
{
    ...
    protected override async Task ProtectedStartAsync(CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            await _db.Ado.ExecuteCommandAsync(sql, default).ConfigureAwait(false);
            ...
            await _db.Ado.ExecuteCommandAsync($"ALTER STABLE `{_driverPropertys.StringTableNameLow}` KEEP 10;", default).ConfigureAwait(false);
            await _db.Ado.ExecuteCommandAsync($"ALTER STABLE `{_driverPropertys.NumberTableNameLow}` KEEP 10;", default).ConfigureAwait(false);

        }
        await base.ProtectedStartAsync(cancellationToken).ConfigureAwait(false);
    }
    public async Task<SqlOrmPagedList<IDBHistoryValue>> GetDBHistoryValuePagesAsync(DBHistoryValuePageInput input)
    {
