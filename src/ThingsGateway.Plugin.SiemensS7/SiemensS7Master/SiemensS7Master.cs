...
public class SiemensS7Master : CollectFoundationBase
{
    ...
    /// <returns></returns>
    public ReadOnlyMemory<byte> GetBytes(DataTypeEnum dataType, JsonNode value)
    {
        return this._plc.BitConverter.GetBytesFromData(value, dataType);
        ...
        static async PooledValueTask<Dictionary<string, OperResult>> WriteValuesAsync(SiemensS7Master @this, Dictionary<VariableRuntime, JsonNode> writeInfoLists, CancellationToken cancellationToken)
        {
            ...
            foreach (var item in w1)
            {
                ...
                if (item.Key.DataType == DataTypeEnum.Boolean)
                {
                    ...
                    {
                        siemensS7Address.BitLength = jArray.Deserialize<Boolean[]>().Length;
                    }
                }
    ...
    protected override async Task<List<VariableSourceRead>> ProtectedLoadSourceReadAsync(List<VariableRuntime> deviceVariables)
    {
        ...
        {
            variableSourceReads.AddRange(_plc.LoadSourceRead<VariableSourceRead, VariableRuntime>(deviceVariable, _driverPropertys.MaxPack, CurrentDevice.IntervalTime));
        }
